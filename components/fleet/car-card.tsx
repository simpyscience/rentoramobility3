...
import { Users, Fuel, Gauge, Star, ArrowRight, CheckCircle2, Sparkles, Luggage, MapPin } from 'lucide-react';
...
export function CarCard({ car, index = 0 }: { car: Car; index?: number }) {
  return (
    ...
    >
      <Link href={`/fleet/${car.slug}`} className="block">
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          <div className="absolute top-3 left-3 flex flex-wrap gap-2">
            ...
            {car.popular && (
              <Badge variant="outline" className="border-white/30 bg-black/30 text-white backdrop-blur-md">
      ...
      <div className="p-5">
        <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{car.tagline}</p>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4 text-center">
          ...
          <Spec icon={Luggage} label={car.specs.luggageCapacity || 'Standard luggage'} />
        </div>

        <div className="flex flex-wrap gap-2 mb-3">
          {car.features.slice(0, 3).map((feature) => (
            <span key={feature} className="rounded-full border border-border bg-muted/40 px-2.5 py-1 text-[11px] text-muted-foreground">
              {feature}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1.5 mb-4 text-[11px] text-muted-foreground">
          ...
          <span>{(car.locations || ['Delhi']).slice(0, 2).join(', ')}</span>
        </div>
        ...
        <div className="flex items-end justify-between pt-4 border-t border-border">
          <div>
            ...
            {car.pricePerHour && (
              <div className="text-xs text-muted-foreground">or ₹{car.pricePerHour.toLocaleString('en-IN')}/hr</div>
            )}
          </div>
...
function Spec({ icon: Icon, label }: { icon: any; label: string }) {
  return (
    <div className="rounded-lg border border-border/70 bg-muted/40 py-2 px-1">
