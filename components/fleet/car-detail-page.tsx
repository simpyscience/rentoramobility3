    ...
    <div className="pt-24 pb-20">
      ...
      <div className="container-lux px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div>
              ...
              <h1 className="font-display text-3xl md:text-5xl font-semibold tracking-tight">{car.name}</h1>
              ...
              {car.tagline && (
                <p className="text-sm text-muted-foreground mt-2 max-w-xl">{car.tagline}</p>
              )}
            ...
            <div className="flex flex-col items-start lg:items-end gap-3">
              <div>
                <div className="text-xs text-muted-foreground uppercase tracking-wider">Starting from</div>
              ...
              <a href={`/book/${car.slug}`} className="hidden lg:inline-block">
        ...
        <div className="grid lg:grid-cols-[1.5fr_1fr] gap-8 mb-12">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }}>
            ...
            <div className="luxury-card p-6 mt-6">
              <h2 className="font-display text-xl font-semibold mb-3">About the {car.name}</h2>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          <div className="luxury-card p-6">
            <h3 className="font-sans text-xl font-semibold mb-4">Features & Amenities</h3>
          ...
          <div className="luxury-card p-6">
            <h3 className="font-sans text-xl font-semibold mb-4">Rental Options</h3>
            <div className="space-y-3 text-sm">
              {car.selfDrive && (
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-gold" /> Self Drive: <span className="font-semibold">Available</span>
                </div>
              )}
              {car.chauffeurAvailable && (
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-gold" /> Chauffeur Driven: <span className="font-semibold">Available</span>
                </div>
              )}
              <div className="flex items-center gap-2">
              ...
              {car.specs.luggageCapacity && (
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-gold" /> Luggage: <span className="font-semibold">{car.specs.luggageCapacity}</span>
                </div>
              )}
            </div>
            {(car.city || (car.locations && car.locations.length > 0)) && (
              <div className="mt-4 pt-4 border-t border-border">
                <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Available in</div>
           ...
           return (
             <div className="mb-12">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                 {routes.map((route) => (
                   <Link key={route.destination.slug} href={`/destinations/${route.destination.slug}`} className="luxury-card p-5 group block">
                     <div className="flex items-center gap-2 mb-2">
                       <span className="font-semibold text-sm">{route.origin}</span>
                       <ArrowRight className="h-4 w-4 text-gold" />
                       <span className="font-semibold text-sm">{route.destination.cityName}</span>
                     </div>
                     <div className="text-xs text-muted-foreground mb-2">
                       {route.travel.distanceKm} km • {route.travel.driveTime}
                     </div>
                     <p className="text-xs text-muted-foreground leading-relaxed line-clamp-2">{route.destination.shortDescription}</p>
                     <div className="inline-flex items-center gap-1 text-xs font-semibold text-gold mt-3">
                       View Route <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" />
                     </div>
                   </Link>
                 ))}
               </div>
